...
export default function PlansPageClient() {
  ...
  const planKeys = ["essential", "premium", "enterprise"]
    ...
    <SharedLayout>
      <div className="min-h-screen bg-background text-white p-5 md:p-10 lg:p-16 grid-pattern">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <button
                onClick={() => { setSelectedPlan(planKeys[index]); setShowForm(true) }}
        ...
        <div className="mt-12 animate-fade-in" style={{ animationDelay: "400ms" }}>
          <button onClick={() => { setSelectedPlan(""); setShowForm(true) }} className="text-white/70 hover:text-white transition-colors minimal-link">
            {t("nav.back")}
          </button>
        </div>
